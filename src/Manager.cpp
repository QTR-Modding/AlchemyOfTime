...
void Manager::UpdateLocationIndexForSource(const Source& src, const RefID location_id) {
    ...
    if (it == src.data.end() || it->second.empty()) {
        ...
        if (src.HasInventoryOwnerTriggers()) {
            QUE_UNIQUE_GUARD;
            RemoveQueuedUpdate(location_id, src.formid, RefStop::Type::kInventoryTriggers);
        }
        return;
    }

    AddLocationIndex(location_id, src.formid);
    if (src.HasInventoryOwnerTriggers()) QueueInventoryUpdate(src, location_id);
}
...
void Manager::ProcessInventoryUpdates() {
    if (!inventory_poll_due_.load(std::memory_order_relaxed) || !inventory_poll_due_.exchange(false)) return;
    if (isLoading.load() || isUninstalled.load()) return;
    if (const auto ui = RE::UI::GetSingleton(); ui && ui->GameIsPaused()) return;
    const auto calendar = RE::Calendar::GetSingleton();
    if (!calendar) return;
    const auto time = calendar->GetHoursPassed();
    for (const auto& info : GetRefStops(RefStop::Type::kInventoryTriggers)) {
        UpdateQueuedRef(info, RefStop::Type::kInventoryTriggers, time);
    }
}
...
void Manager::UpdateLoop() {
    ...
    if (QUE_UNIQUE_GUARD;
        ...
        for (auto it = _ref_stops_.begin(); it != _ref_stops_.end();) {
            if (it->second.type != RefStop::Type::kWorldObject) {
                ++it;
                continue;
            }
            if (const auto ref = it->second.GetRef();
                queue_delete_.contains(it->second.ref_info.ref_id) ||
    ...
    {
        QUE_SHARED_GUARD;
        bool inventory_pending = false;
        for (const auto& stop : _ref_stops_ | std::views::values) {
            if (stop.type == RefStop::Type::kWorldObject) {
                if (ref_stops_copy.empty()) ref_stops_copy.reserve(_ref_stops_.size());
                ref_stops_copy.push_back(stop.ref_info);
            }
            else if (stop.type == RefStop::Type::kInventoryTriggers) inventory_pending = true;
        }
        if (inventory_pending) inventory_poll_due_.store(true, std::memory_order_relaxed);
    }
    ...
    if (const auto cal = RE::Calendar::GetSingleton()) {
        ...
        std::vector<std::pair<RefID, FormID>> ref_stops_due;
        ref_stops_due.reserve(ref_stops_copy.size());
        for (
            QUE_UNIQUE_GUARD;
            const auto& key : ref_stops_copy) {
            auto stop = GetRefStop(key.ref_id);
            if (!stop) continue;

            if (auto& val = *stop; val.IsDue(curr_time)) {
                ref_stops_due.emplace_back(key.ref_id, key.source_id);
        ...
        for (QUE_UNIQUE_GUARD; const auto& [refid, source] : ref_stops_due) {
            RemoveQueuedUpdate(refid, source, RefStop::Type::kWorldObject);
        }
    }
    ...
    if (curr_time > 0.f) {
        for (const auto& ref_info : ref_stops_copy) {
            M->UpdateQueuedRef(ref_info, RefStop::Type::kWorldObject, curr_time);
        }
    }
    //});
}
...
void Manager::QueueRefUpdate(const RefStop& a_refstop) {
    if (a_refstop.type == RefStop::Type::kWorldObject && !Settings::world_objects_evolve.load()) return;

    bool needStart;
    {
        const auto refid = a_refstop.ref_info.ref_id;
        QUE_UNIQUE_GUARD;
        const UpdateKey key{refid, a_refstop.type == RefStop::Type::kWorldObject ? 0 : a_refstop.ref_info.source_id,
                            a_refstop.type};
        if (auto [it, inserted] = _ref_stops_.try_emplace(key, a_refstop); !inserted) {
...
void Manager::RemoveQueuedUpdate(const RefID refid, const FormID source, const RefStop::Type type) {
    const UpdateKey key{refid, type == RefStop::Type::kWorldObject ? 0 : source, type};
    if (const auto it = _ref_stops_.find(key); it != _ref_stops_.end() && it->second.ref_info.source_id == source) {
        if (type == RefStop::Type::kWorldObject) PreDeleteRefStop(it->second);
        _ref_stops_.erase(it);
    }
}

void Manager::QueueInventoryUpdate(const Source& source, const RefID owner) {
    const auto it = source.data.find(owner);
    if (it == source.data.end() || !std::ranges::any_of(it->second, [](const auto& instance) {
            return instance.count > 0 && !instance.xtra.is_decayed;
        })) {
        QUE_UNIQUE_GUARD;
        RemoveQueuedUpdate(owner, source.formid, RefStop::Type::kInventoryTriggers);
        return;
    }
    RefStop stop(owner);
    const auto ref = stop.GetRef();
    if (!ref || !ref->HasContainer() || !ref->Is(RE::FormType::ActorCharacter)) return;
    stop.ref_info.source_id = source.formid;
    stop.type = RefStop::Type::kInventoryTriggers;
    QueueRefUpdate(stop);
}

void Manager::UpdateQueuedRef(const RefInfo& info, const RefStop::Type type, const float time) {
    switch (type) {
    case RefStop::Type::kWorldObject:
        UpdateQueuedWO(info, time);
        return;
    case RefStop::Type::kInventoryTriggers: {
        SRC_SHARED_GUARD;
        const auto src = sources.find(info.source_id);
        const auto owner = info.GetRef();
        if (src == sources.end() || !src->second->IsHealthy() || !src->second->HasInventoryOwnerTriggers() ||
            !owner || owner->IsDeleted() || owner->IsMarkedForDeletion() || !owner->HasContainer()) {
            QUE_UNIQUE_GUARD;
            RemoveQueuedUpdate(info.ref_id, info.source_id, type);
            return;
        }
        const auto instances = src->second->data.find(info.ref_id);
        ...
        if (src->second->InventoryTriggersNeedUpdate(info, *owner, time)) MarkDirty_(owner);
        return;
    }
    }
}
...
RefStop* Manager::GetRefStop(const RefID refid) {
    const auto it = _ref_stops_.find({refid, 0, RefStop::Type::kWorldObject});
...
bool Manager::DeRegisterRef(const RefID refid) {
    ...
    for (auto& src : sources | std::views::values) {
        ...
        if (auto it = source.data.find(refid); it != source.data.end()) {
            ...
            source.data.erase(it);
            if (source.HasInventoryOwnerTriggers()) {
                QUE_UNIQUE_GUARD;
                RemoveQueuedUpdate(refid, source.formid, RefStop::Type::kInventoryTriggers);
            }
...
void Manager::Reset() {
    ...
    ClearWOUpdateQueue();
    {
        ...
        _ref_stops_.clear();
        queue_delete_.clear();
        inventory_poll_due_.store(false);
    }
...
std::map<std::tuple<RefID, FormID, RefStop::Type>, float> Manager::GetUpdateQueue() {
    std::map<std::tuple<RefID, FormID, RefStop::Type>, float> ref_stops_copy;
    QUE_SHARED_GUARD;
    for (const auto& value : _ref_stops_ | std::views::values) {
        ref_stops_copy[{value.ref_info.ref_id, value.ref_info.source_id, value.type}] = value.stop_time;
    }
...
std::vector<RefInfo> Manager::GetRefStops(const RefStop::Type type) {
    std::vector<RefInfo> ref_stops_copy;
    QUE_SHARED_GUARD;
    for (const auto& refstop : _ref_stops_ | std::views::values) {
        if (refstop.type != type) continue;
